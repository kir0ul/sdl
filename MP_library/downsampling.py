        ...
        for seg in range(len(inds) - 1):
            ...
            for i in range(inds[seg], inds[seg + 1]):
                ...
                d = perpendicularDistance(
                    PointList[i], ResultList[seg], ResultList[seg + 1]
                )
                if d > dmax:
...
if __name__ == "__main__":
    ...
    (naive,) = plt.plot(
        traj_naive[:, 0], traj_naive[:, 1], "m.-", lw=2, ms=13, label="Naive"
    )
    (db,) = plt.plot(
        traj_db[:, 0], traj_db[:, 1], "gx-", lw=2, ms=13, mew=3, label="Distance-based"
    )
    (dp,) = plt.plot(
        traj_dp[:, 0], traj_dp[:, 1], "y*-", lw=2, ms=13, label="Douglas-Peucker"
    )
    plt.xticks([])
    plt.yticks([])
