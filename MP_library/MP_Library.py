...
class MP_Library(object):
    ...
    def add_primitive(self, demo, name=None, force_new_class=False):
        similarities = []
        out = None
        ...
        if (
            len(similarities) > 0
            and min(similarities) < self.threshold
            and not force_new_class
        ):
                    ...
                    for dim_i in range(n_dims - 1):
                        ...
                        if dim_i == 0:
                            axs[skill_i, dim_i].set_ylabel(
                                class_key.replace("_", " "), fontsize=label_fontsize
                            )
                        if skill_i == 0:
                            axs[skill_i, dim_i].set_title(
                                f"Dimension {dim_i}", fontsize=label_fontsize
                            )
                        if skill_i == n_classes - 1:
                            axs[skill_i, dim_i].set_xlabel(
                                "Time steps", fontsize=label_fontsize
                            )
